...
window.addEventListener('deviceorientation', (event) => {
    ...
    tiltX = event.gamma / 90; // Left-right tilt (-1 to 1)
    tiltY = event.beta / 90;  // Forward-back tilt (-1 to 1)
});

// Swipe handling for devices without gyroscope
let touchStartY = 0;
...
document.addEventListener('touchend', (e) => {
    ...
    if (touchStartY - touchEndY > 50) { // Swipe up
        leaves.forEach(leaf => {
            leaf.applyForce(0, -5); // Impulse upward
        });
    }
});
